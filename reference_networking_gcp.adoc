...
+
単一ノードシステムと同様に、共有 VPC とスタンドアロン VPC で HA ペアをサポートします。ただし、共有 VPC にできるのは VPC -0 のみです。それ以外の VPC はすべてスタンドアロン VPC である必要があります。
...
+
Cloud Manager では、 GCP のシングルノードシステム用の SVM 管理 LIF は作成されません。
...
+
** VPC 0 の IP アドレス × 10
** VPC 1 用の IP アドレスが 2 つあります
** VPC 2 の IP アドレスが 2 つあります
** VPC 3 つの IP アドレス
