...
+
単一ノードシステムと同様に、共有 VPC とスタンドアロン VPC で HA ペアをサポートします。どの VPC でも、共有 VPC または非共有 VPC を使用できます。
...
* * HA ペア * ： 15 または 16 個の IP アドレス
+
** VPC 0 の IP アドレスは 8 個または 9 個です
+
Cloud Volumes ONTAP を API を使用して導入する場合、 Storage VM （ SVM ）管理 LIF の作成をスキップし、次のフラグを指定できます。
