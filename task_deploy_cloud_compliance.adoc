...
summary: Cloud Manager のワークスペースに Cloud Data Sense インスタンスを導入するには、いくつかの手順を実行します。クラウドまたはオンプレミスシステムにデータセンスを導入できます。 
---
= クラウドデータの導入センス
:hardbreaks:
:nofooter: 
...
* クラウドにインストールした場合は、 Cloud Data Sense インスタンス用の vCPU が 16 個必要です。を参照してください link:concept_cloud_compliance.html#the-cloud-data-sense-instance["インスタンスタイプの詳細"^]。
