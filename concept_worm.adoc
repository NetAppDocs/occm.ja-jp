...
* Cloud Volumes ONTAP の WORM ストレージは、「信頼されたストレージ管理者」モデルで機能します。WORM ファイルおよび Snapshot は書き換えから保護されますが、ボリュームに期限切れ前の WORM データが含まれていても、クラスタ管理者はボリュームを削除できます。
+
たとえば、ディスクをクラウドプロバイダから直接削除または移動した場合、ボリュームは有効期限が切れる前に削除できます。
