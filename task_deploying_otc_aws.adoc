...
. * 場所と接続 * ：に記録したネットワーク情報を入力します link:task_planning_your_config.html#aws-network-information-worksheet["AWS ワークシート"]。
+
