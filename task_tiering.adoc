...
* AWS では、パフォーマンス階層は汎用 SSD （ gp2 ）、プロビジョニングされた IOPS SSD 、またはスループット最適化 HDD にすることができます。
...
.手順
. Cloud Manager コンソールの右上にある * Settings * アイコンをクリックし、 * Cloud Manager Settings * を選択します。
+
image:screenshot_settings_icon.gif["Cloud Manager コンソールの右上にある設定アイコンを示すスクリーンショット。"]

. [ アグリゲート容量のしきい値 ] で、 [ データ階層化のアグリゲートの空きスペース率 *] をクリックします。
