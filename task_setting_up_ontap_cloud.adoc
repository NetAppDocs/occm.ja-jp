...
| オプション： Cloud Manager Connector を AutoSupport プロキシとして設定  a| 
プロキシサーバで AutoSupport メッセージを送信する必要がある環境では、プロキシとして機能するようにコネクタを設定できます。インターネットアクセス以外のコネクタの設定は必要ありません。Cloud Volumes ONTAP の CLI にアクセスして、次のコマンドを実行するだけです。
